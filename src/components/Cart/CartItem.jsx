  ...
  return (
    <div className="py-5 border-b border-base-300 last:border-b-0">
      <div className="flex items-start justify-between">
        ...
        <button onClick={() => removeItemFromCart(item.id)} className="text-red-500 hover:text-red-700 ml-4 flex-shrink-0">
          <Trash2 size={20} />
        </button>
      </div>
      <div className="flex items-center justify-between mt-4">
        <div className="flex items-center border border-base-300 rounded-full">
          <button onClick={() => updateItemQuantity(item.id, item.quantity - 1)} className="p-2 text-neutral hover:text-primary transition-colors"><Minus size={16}/></button>
          <span className="px-4 font-bold text-sm">{item.quantity}</span>
          <button onClick={() => updateItemQuantity(item.id, item.quantity + 1)} className="p-2 text-neutral hover:text-primary transition-colors"><Plus size={16}/></button>
        </div>
